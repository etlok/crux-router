...
export class WebSocketController {
  ...
  async validateToken(@Body() tokenDto: TokenValidationDto) {
    try {
      ...
      const payload = await this.clientAuthService.validateToken(
        tokenDto.token,
      );
  ...
  async broadcastMessage(@Body() broadcastDto: BroadcastDto) {
    try {
      ...
      if (broadcastDto.room) {
        ...
        this.logger.log(
          `Broadcasting to room ${broadcastDto.room}: ${JSON.stringify(eventData)}`,
        );
        ...
        this.logger.log(
          `Broadcasting to client ${broadcastDto.clientId}: ${JSON.stringify(eventData)}`,
        );
        ...
        this.logger.log(
          `Broadcasting to all clients: ${JSON.stringify(eventData)}`,
        );
  ...
  getConnections() {
    ...
    const authenticatedConnections =
      this.wsGateway.getAuthenticatedClientsCount();
  ...
  async initialize(@Body() initializeDto: InitializeDto, @Req() req: any) {
    try {
      this.logger.log(
        `Initialize request received: ${JSON.stringify(initializeDto)}`,
      );
      ...
      if (
        !initializeDto.payload ||
        !Array.isArray(initializeDto.payload.channels)
      ) {
        throw new BadRequestException(
          'Invalid payload format. Channels array is required.',
        );
      }
      ...
      if (req.isAuthenticated && req.user) {
        ...
        this.logger.log(
          `User authenticated by middleware: ${authenticatedUserId}`,
        );
      }
      // Fallback to manual token validation if middleware didn't authenticate
        ...
        try {
          const payload = await this.clientAuthService.validateToken(
            initializeDto.auth.token,
          );
          authenticatedUserId = payload.sub || payload.id;
          this.logger.log(
            `User authenticated manually: ${authenticatedUserId}`,
          );
        } catch (error) {
  ...
  async channelBroadcast(
    @Body() broadcastDto: ChannelBroadcastDto,
    @Req() req: any,
  ) {
    try {
      this.logger.log(
        `Broadcast request received: ${JSON.stringify(broadcastDto)}`,
      );
      ...
      if (
        !broadcastDto.channel_ids ||
        !Array.isArray(broadcastDto.channel_ids) ||
        broadcastDto.channel_ids.length === 0
      ) {
        throw new BadRequestException(
          'Invalid request. At least one channel_id is required.',
        );
      }
      ...
      if (req.isAuthenticated && req.user) {
        this.logger.log(
          `Broadcast from authenticated user: ${req.user.sub || req.user.id}`,
        );
      ...
      for (const channelId of broadcastDto.channel_ids) {
        const channelExists = await this.redisService.exists(
          `channel:${channelId}`,
        );
        if (!channelExists) {
          this.logger.warn(
            `Channel ${channelId} does not exist, but will attempt to broadcast anyway`,
          );
        }
      }
