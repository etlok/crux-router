...
export class MiddlewareConfigService implements OnModuleInit {
  ...
  async loadConfigFromFileToRedis(configPath?: string): Promise<void> {
    ...
    try {
      ...
      const existingKeys = await redisClient.keys(`${REDIS_MIDDLEWARE_KEYS.CONFIG_ITEM_PREFIX}*`);
  ...
  async getConfigByKey(key: string): Promise<MiddlewareConfig | null> {
    try {
      const redisClient = await this.redisService.getPubClient();
      const configJson = await redisClient.get(`${REDIS_MIDDLEWARE_KEYS.CONFIG_ITEM_PREFIX}${key}`);
  ...
  async setActiveMiddleware(keys: string[]): Promise<void> {
    try {
      ...
      await redisClient.set(REDIS_MIDDLEWARE_KEYS.ACTIVE_MIDDLEWARE, JSON.stringify(keys));
  ...
  async getActiveMiddleware(): Promise<string[]> {
    try {
      ...
      const activeJson = await redisClient.get(REDIS_MIDDLEWARE_KEYS.ACTIVE_MIDDLEWARE);
