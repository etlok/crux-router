...
export class MiddlewareLoaderService {
   ...
   */
  registerMiddleware(key: string, middlewareInstance: MiddlewareFunction, priority: number = 100): void {
   ...
   */
  async initializeMiddleware(middlewarePath: string = path.join(__dirname, 'middlewares')): Promise<void> {
    if (this.initialized) {
      return;
    }
    ...
    try {
      ...
      const classConfigs = configs.filter(c => c.type === 'class') as ClassMiddlewareConfig[];
      ...
      for (const config of classConfigs) {
        try {
          ...
          const createBasicMiddleware = (middlewareName: string) => {
            return {
              [middlewareName + 'Middleware']: class implements MiddlewareFunction {
                constructor() {}
  ...
  private async executeDefaultMiddlewareChain(context: any): Promise<void> {
    ...
    const defaultMiddlewares = this.middlewares.filter(m => 
      m.enabled && ['logging', 'authentication'].includes(m.name)
    );
